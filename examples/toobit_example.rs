//! Toobit REST API Example
//!
//! Demonstrates using the Toobit exchange REST API
//! Run with: cargo run --example toobit_example

use ccxt_rust::exchanges::foreign::Toobit;
use ccxt_rust::types::Exchange;
use ccxt_rust::ExchangeConfig;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Toobit Exchange REST API Example");
    println!("=================================\n");

    // Create exchange instance with longer timeout
    let config = ExchangeConfig::default().with_timeout(60000);
    let exchange = Toobit::new(config)?;
    ...
    match exchange.fetch_trades("BTC/USDT", None, Some(5)).await {
        Ok(trades) => {
