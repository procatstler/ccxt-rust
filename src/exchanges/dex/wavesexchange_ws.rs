...
impl WavesexchangeWs {
    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(msg_type) = json.get("T").and_then(|t| t.as_str()) {
            let symbol = json.get("S").and_then(|s| s.as_str())
            ...
            match msg_type {
                ...
                "t" => {
                    ...
                    if let Some(trades) = json.get("o") {
                        if let Ok(trades_data) = serde_json::from_value::<Vec<WavesWsTrade>>(trades.clone()) {
...
impl Default for WavesexchangeWs { fn default() -> Self { Self::new() } }
...
impl WsExchange for WavesexchangeWs {
    ...
    async fn watch_order_book(&self, symbol: &str, _limit: Option<u32>) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        let mut ws = self.clone();
        ws.subscribe_stream("ob", &Self::format_symbol(symbol)).await
    }
...
struct WavesWsTicker {
    ...
    #[serde(default, rename = "lp")] last_price: Option<Decimal>,
    #[serde(default, rename = "fp")] first_price: Option<Decimal>,
    #[serde(default, rename = "v")] volume: Option<Decimal>,
    #[serde(default, rename = "vw")] volume_waves: Option<Decimal>,
}
...
struct WavesWsOrderBook {
    #[serde(default)] timestamp: Option<i64>,
    #[serde(default, rename = "b")] bids: Vec<WavesWsOrderEntry>,
    #[serde(default, rename = "a")] asks: Vec<WavesWsOrderEntry>,
}
...
struct WavesWsTrade {
    ...
    #[serde(default, rename = "p")] price: Option<Decimal>,
    #[serde(default, rename = "a")] amount: Option<Decimal>,
    #[serde(default, rename = "ot")] order_type: Option<String>,
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(WavesexchangeWs::format_symbol("WAVES/USDT"), "WAVES/USDT"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(WavesexchangeWs::to_unified_symbol("WAVES/USDT"), "WAVES/USDT"); }
    #[test] fn test_default() { let ws = WavesexchangeWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = WavesexchangeWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = WavesexchangeWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = WavesexchangeWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = WavesWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(5)),
            low: Some(Decimal::from(4)), bid: Some(Decimal::from(4)),
            ask: Some(Decimal::from(5)), last_price: Some(Decimal::from(4)),
            first_price: Some(Decimal::from(4)), volume: Some(Decimal::from(1000000)),
    ...
    fn test_parse_order_book() {
        let data = WavesWsOrderBook {
            ...
            bids: vec![WavesWsOrderEntry { price: Some(Decimal::from(4)), amount: Some(Decimal::from(100)) }],
            asks: vec![WavesWsOrderEntry { price: Some(Decimal::from(5)), amount: Some(Decimal::from(50)) }]
        };
    ...
    fn test_parse_trade() {
        let data = WavesWsTrade {
            id: Some("abc123".into()), timestamp: Some(1704067200000),
            price: Some(Decimal::from(4)), amount: Some(Decimal::from(100)),
