            ...
            if let Some(data) = json.get("data") {
                if channel.contains("ticker") {
                    if let Some(first) = data.as_array().and_then(|arr| arr.first()) {
                        if let Ok(ticker_data) = serde_json::from_value::<DeepcoinWsTicker>(first.clone()) {
                    ...
                    if let Some(first) = data.as_array().and_then(|arr| arr.first()) {
                        if let Ok(book_data) = serde_json::from_value::<DeepcoinWsOrderBook>(first.clone()) {
                    ...
                    if let Ok(trades_data) = serde_json::from_value::<Vec<DeepcoinWsTrade>>(data.clone()) {
...
impl Default for DeepcoinWs { fn default() -> Self { Self::new() } }
...
impl WsExchange for DeepcoinWs {
    async fn watch_ticker(&self, symbol: &str) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        ...
        ws.subscribe_stream("tickers", &Self::format_symbol(symbol)).await
    }
    async fn watch_order_book(&self, symbol: &str, _limit: Option<u32>) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        let mut ws = self.clone();
        ws.subscribe_stream("books", &Self::format_symbol(symbol)).await
    }
...
struct DeepcoinWsTicker {
    ...
    #[serde(default, rename = "bestBidSz")] best_bid_size: Option<Decimal>,
    #[serde(default, rename = "bestAsk")] best_ask: Option<Decimal>,
    #[serde(default, rename = "bestAskSz")] best_ask_size: Option<Decimal>,
    #[serde(default, rename = "open24h")] open_24h: Option<Decimal>,
    #[serde(default)] last: Option<Decimal>,
    #[serde(default, rename = "change24h")] change_24h: Option<Decimal>,
    #[serde(default, rename = "vol24h")] volume_24h: Option<Decimal>,
    #[serde(default, rename = "turnover24h")] turnover_24h: Option<Decimal>,
    #[serde(default, rename = "idxPx")] index_price: Option<Decimal>,
    #[serde(default, rename = "markPx")] mark_price: Option<Decimal>,
}
...
struct DeepcoinWsTrade {
    ...
    #[serde(default)] timestamp: Option<i64>,
    #[serde(default, rename = "px")] price: Option<Decimal>,
    #[serde(default, rename = "sz")] size: Option<Decimal>,
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(DeepcoinWs::format_symbol("BTC/USDT"), "BTC-USDT"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(DeepcoinWs::to_unified_symbol("BTC-USDT"), "BTC/USDT"); }
    #[test] fn test_default() { let ws = DeepcoinWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = DeepcoinWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = DeepcoinWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = DeepcoinWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = DeepcoinWsTicker {
            ...
            best_bid_size: Some(Decimal::from(2)), best_ask: Some(Decimal::from(44600)),
            best_ask_size: Some(Decimal::from(1)), open_24h: Some(Decimal::from(44000)),
            last: Some(Decimal::from(44550)), change_24h: None,
            volume_24h: Some(Decimal::from(100)), turnover_24h: None,
    ...
    fn test_parse_trade() {
        let data = DeepcoinWsTrade {
            ...
            price: Some(Decimal::from(44550)), size: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
