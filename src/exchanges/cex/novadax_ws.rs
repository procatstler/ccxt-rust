        ...
        if let Some(topic) = json.get("topic").and_then(|t| t.as_str()) {
            ...
            if let Some(data) = json.get("data") {
                if topic.contains("ticker") {
                    ...
                    if let Ok(book_data) = serde_json::from_value::<NovadaxWsOrderBook>(data.clone()) {
                    ...
                    if let Ok(trades_data) = serde_json::from_value::<Vec<NovadaxWsTrade>>(data.clone()) {
                    ...
                    } else if let Ok(trade_data) = serde_json::from_value::<NovadaxWsTrade>(data.clone()) {
...
impl Default for NovadaxWs { fn default() -> Self { Self::new() } }
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(NovadaxWs::format_symbol("BTC/BRL"), "BTC_BRL"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(NovadaxWs::to_unified_symbol("BTC_BRL"), "BTC/BRL"); }
    #[test] fn test_default() { let ws = NovadaxWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = NovadaxWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = NovadaxWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = NovadaxWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = NovadaxWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(350000)),
            low: Some(Decimal::from(340000)), best_bid: Some(Decimal::from(345000)),
            best_ask: Some(Decimal::from(346000)), last_price: Some(Decimal::from(345500)),
            open: Some(Decimal::from(344000)), change: None, base_volume: Some(Decimal::from(100)),
    ...
    fn test_parse_trade() {
        let data = NovadaxWsTrade {
            ...
            price: Some(Decimal::from(345500)), amount: Some(Decimal::from(1)),
            side: Some("BUY".into())
        };
