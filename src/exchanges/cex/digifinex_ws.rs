                ...
                if method.contains("ticker") {
                    if let Some(data) = params.get(1) {
                        if let Ok(ticker_data) =
                            serde_json::from_value::<DigifinexWsTicker>(data.clone())
                        {
                    ...
                    if let Some(data) = params.get(1) {
                        if let Ok(book_data) =
                            serde_json::from_value::<DigifinexWsOrderBook>(data.clone())
                        {
                    ...
                    if let Some(data) = params.get(1) {
                        if let Ok(trades_data) =
                            serde_json::from_value::<Vec<DigifinexWsTrade>>(data.clone())
                        {
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(DigifinexWs::format_symbol("BTC/USDT"), "btc_usdt");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(DigifinexWs::to_unified_symbol("btc_usdt"), "BTC/USDT");
    }
    #[test]
    fn test_default() {
        let ws = DigifinexWs::default();
        assert!(ws.ws_client.is_none());
    }
    #[test]
    fn test_clone() {
        let ws = DigifinexWs::new();
    ...
    fn test_parse_ticker() {
        let data = DigifinexWsTicker {
            ...
            change: None,
            base_vol: Some(Decimal::from(100)),
            quote_vol: Some(Decimal::from(4450000)),
        };
