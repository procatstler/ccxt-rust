...
impl BtcturkWs {
    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            let pair = json.get("pairSymbol").and_then(|p| p.as_str()).unwrap_or("");
            ...
            if let Some(data) = json.get("data") {
                if channel.contains("ticker") {
                    ...
                    if let Ok(book_data) = serde_json::from_value::<BtcturkWsOrderBook>(data.clone()) {
                    ...
                    if let Ok(trades_data) = serde_json::from_value::<Vec<BtcturkWsTrade>>(data.clone()) {
                    ...
                    } else if let Ok(trade_data) = serde_json::from_value::<BtcturkWsTrade>(data.clone()) {
...
impl Default for BtcturkWs { fn default() -> Self { Self::new() } }
...
struct BtcturkWsTicker {
    ...
    #[serde(default)] average: Option<Decimal>,
    #[serde(default, rename = "dailyChange")] daily_change: Option<Decimal>,
    #[serde(default, rename = "dailyChangePercent")] daily_change_percent: Option<Decimal>,
}
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(BtcturkWs::format_symbol("BTC/TRY"), "BTCTRY"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(BtcturkWs::to_unified_symbol("BTCTRY"), "BTC/TRY"); }
    #[test] fn test_default() { let ws = BtcturkWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = BtcturkWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = BtcturkWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = BtcturkWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = BtcturkWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(1500000)),
            low: Some(Decimal::from(1400000)), bid: Some(Decimal::from(1450000)),
            ask: Some(Decimal::from(1455000)), last: Some(Decimal::from(1452000)),
            open: Some(Decimal::from(1440000)), volume: Some(Decimal::from(100)),
            average: None, daily_change: None, daily_change_percent: None
        };
    ...
    fn test_parse_trade() {
        let data = BtcturkWsTrade {
            ...
            price: Some(Decimal::from(1452000)), amount: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
