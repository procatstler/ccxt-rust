                ...
                match event {
                    "ticker" => {
                        if let Ok(ticker_data) =
                            serde_json::from_value::<OceanexWsTicker>(data.clone())
                        {
                    ...
                    "orderbook" | "depth" => {
                        if let Ok(book_data) =
                            serde_json::from_value::<OceanexWsOrderBook>(data.clone())
                        {
                    ...
                    "trades" | "trade" => {
                        if let Ok(trades_data) =
                            serde_json::from_value::<Vec<OceanexWsTrade>>(data.clone())
                        {
                            ...
                            serde_json::from_value::<OceanexWsTrade>(data.clone())
                        {
...
struct OceanexWsTicker {
    ...
    at: Option<i64>,
    #[serde(default)]
    ticker: Option<OceanexWsTickerInfo>,
}
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(OceanexWs::format_symbol("BTC/USDT"), "btcusdt");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(OceanexWs::to_unified_symbol("btcusdt"), "BTC/USDT");
    }
    #[test]
    fn test_default() {
        let ws = OceanexWs::default();
        assert!(ws.ws_client.is_none());
    }
    #[test]
    fn test_clone() {
        let ws = OceanexWs::new();
    ...
    fn test_parse_ticker() {
        ...
        let data = OceanexWsTicker {
            at: Some(1704067200),
            ticker: Some(ticker_info),
        };
