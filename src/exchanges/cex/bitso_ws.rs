        ...
        if let Some(msg_type) = json.get("type").and_then(|t| t.as_str()) {
            ...
            match msg_type {
                ...
                "orders" | "diff-orders" => {
                    if let Some(payload) = json.get("payload") {
                        if let Ok(data) =
                            serde_json::from_value::<BitsoWsOrderBook>(payload.clone())
                        {
                ...
                "trades" => {
                    if let Some(payload) = json.get("payload") {
                        if let Ok(trades_data) =
                            serde_json::from_value::<Vec<BitsoWsTrade>>(payload.clone())
                        {
...
impl WsExchange for BitsoWs {
    ...
    async fn watch_order_book(
        ...
        let mut ws = self.clone();
        ws.subscribe_stream("orders", &Self::format_symbol(symbol))
            .await
    }
...
struct BitsoWsOrderBook {
    #[serde(default)]
    bids: Vec<BitsoWsOrderBookEntry>,
    #[serde(default)]
    asks: Vec<BitsoWsOrderBookEntry>,
}
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(BitsoWs::format_symbol("BTC/MXN"), "btc_mxn");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(BitsoWs::to_unified_symbol("btc_mxn"), "BTC/MXN");
    }
    #[test]
    fn test_default() {
        let ws = BitsoWs::default();
    ...
    fn test_parse_ticker() {
        let data = BitsoWsTicker {
            high: Some(Decimal::from(1000000)),
            low: Some(Decimal::from(950000)),
            bid: Some(Decimal::from(980000)),
            ask: Some(Decimal::from(985000)),
            last: Some(Decimal::from(982000)),
            vwap: None,
            volume: Some(Decimal::from(50)),
    ...
    fn test_parse_trade() {
        let data = BitsoWsTrade {
            tid: Some(123),
            price: Some(Decimal::from(982000)),
            amount: Some(Decimal::from(1)),
            maker_side: Some("buy".into()),
            created_at: Some("2024-01-01T00:00:00Z".into()),
        };
