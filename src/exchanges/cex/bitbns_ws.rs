    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(event_type) = json.get("type").and_then(|t| t.as_str()) {
            ...
            if let Some(data) = json.get("data") {
                match event_type {
                    "ticker" => {
                        if let Ok(ticker_data) = serde_json::from_value::<BitbnsWsTicker>(data.clone()) {
                    ...
                    "orderbook" | "depth" => {
                        if let Ok(book_data) = serde_json::from_value::<BitbnsWsOrderBook>(data.clone()) {
                    ...
                    "trades" | "trade" => {
                        if let Ok(trades_data) = serde_json::from_value::<Vec<BitbnsWsTrade>>(data.clone()) {
                        ...
                        } else if let Ok(trade_data) = serde_json::from_value::<BitbnsWsTrade>(data.clone()) {
...
impl Default for BitbnsWs { fn default() -> Self { Self::new() } }
...
struct BitbnsWsTicker {
    ...
    #[serde(default, rename = "highestBuyBid")] highest_buy_bid: Option<Decimal>,
    #[serde(default, rename = "lowestSellBid")] lowest_sell_bid: Option<Decimal>,
    #[serde(default, rename = "lastTradedPrice")] last_traded_price: Option<Decimal>,
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(BitbnsWs::format_symbol("BTC/INR"), "BTC"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(BitbnsWs::to_unified_symbol("BTC"), "BTC/INR"); }
    #[test] fn test_default() { let ws = BitbnsWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = BitbnsWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = BitbnsWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = BitbnsWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = BitbnsWsTicker {
            high: Some(Decimal::from(5000000)), low: Some(Decimal::from(4800000)),
            highest_buy_bid: Some(Decimal::from(4950000)), lowest_sell_bid: Some(Decimal::from(4960000)),
            last_traded_price: Some(Decimal::from(4955000)), volume: Some(Decimal::from(100))
        };
