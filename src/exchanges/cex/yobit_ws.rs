        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            ...
            if parts.len() >= 2 {
                ...
                if let Some(data) = json.get("data") {
                    match channel_type {
                        "ticker" => {
                            if let Ok(ticker_data) = serde_json::from_value::<YobitWsTicker>(data.clone()) {
                        ...
                        "depth" | "orderbook" => {
                            if let Ok(book_data) = serde_json::from_value::<YobitWsOrderBook>(data.clone()) {
                        ...
                        "trades" => {
                            if let Ok(trades_data) = serde_json::from_value::<Vec<YobitWsTrade>>(data.clone()) {
                            ...
                            } else if let Ok(trade_data) = serde_json::from_value::<YobitWsTrade>(data.clone()) {
...
impl Default for YobitWs { fn default() -> Self { Self::new() } }
...
struct YobitWsTicker {
    #[serde(default)] updated: Option<i64>,
    ...
    #[serde(default)] avg: Option<Decimal>,
    #[serde(default)] vol: Option<Decimal>,
    #[serde(default)] vol_cur: Option<Decimal>,
}
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(YobitWs::format_symbol("BTC/USD"), "btc_usd"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(YobitWs::to_unified_symbol("btc_usd"), "BTC/USD"); }
    #[test] fn test_default() { let ws = YobitWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = YobitWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = YobitWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = YobitWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = YobitWsTicker {
            updated: Some(1704067200), high: Some(Decimal::from(45000)),
            low: Some(Decimal::from(43000)), buy: Some(Decimal::from(44500)),
            sell: Some(Decimal::from(44600)), last: Some(Decimal::from(44550)),
            avg: Some(Decimal::from(44000)), vol: Some(Decimal::from(100)),
    ...
    fn test_parse_trade() {
        let data = YobitWsTrade {
            tid: Some(123), timestamp: Some(1704067200),
