//! Coinbase Advanced Exchange Implementation
//!
//! Alias/wrapper for Coinbase exchange (both use the same Advanced Trade API)
...
/// Coinbase Advanced 거래소 (Coinbase 래퍼)
pub struct CoinbaseAdvanced(Coinbase);

impl CoinbaseAdvanced {
    /// 새 Coinbase Advanced 인스턴스 생성
    pub fn new(config: ExchangeConfig) -> CcxtResult<Self> {
        Ok(Self(Coinbase::new(config)?))
    }
}
...
mod tests {
    ...
    fn test_exchange_info() {
        ...
        assert_eq!(exchange.name(), "Coinbase Advanced");
