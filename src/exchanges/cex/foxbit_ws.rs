    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            if let Some(data) = json.get("data") {
                ...
                match channel {
                    "ticker" => {
                        if let Ok(ticker_data) = serde_json::from_value::<FoxbitWsTicker>(data.clone()) {
                    ...
                    "orderbook" | "depth" => {
                        if let Ok(book_data) = serde_json::from_value::<FoxbitWsOrderBook>(data.clone()) {
                    ...
                    "trades" | "trade" => {
                        if let Ok(trades_data) = serde_json::from_value::<Vec<FoxbitWsTrade>>(data.clone()) {
                        ...
                        } else if let Ok(trade_data) = serde_json::from_value::<FoxbitWsTrade>(data.clone()) {
        ...
        if let Some(event) = json.get("event").and_then(|e| e.as_str()) {
            if let Some(data) = json.get("data") {
                let market = data.get("symbol").and_then(|s| s.as_str())
                    .or_else(|| data.get("market").and_then(|m| m.as_str()))
                    .unwrap_or("");
                let symbol = Self::to_unified_symbol(market);
                ...
                match event {
                    ...
                    "trade" | "trades" => {
                        if let Ok(trade_data) = serde_json::from_value::<FoxbitWsTrade>(data.clone()) {
                            let trade = Self::parse_trade(&trade_data, &symbol);
                            let _ = event_tx.send(WsMessage::Trade(WsTradeEvent { symbol: symbol.clone(), trades: vec![trade] }));
                        }
                    }
...
impl Default for FoxbitWs { fn default() -> Self { Self::new() } }
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(FoxbitWs::format_symbol("BTC/BRL"), "btcbrl"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(FoxbitWs::to_unified_symbol("btcbrl"), "BTC/BRL"); }
    #[test] fn test_default() { let ws = FoxbitWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = FoxbitWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = FoxbitWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = FoxbitWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = FoxbitWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(250000)),
            low: Some(Decimal::from(240000)), bid: Some(Decimal::from(248000)),
            ask: Some(Decimal::from(249000)), last: Some(Decimal::from(248500)),
            open: Some(Decimal::from(245000)), change: None, change_percent: None,
            volume: Some(Decimal::from(100)), quote_volume: Some(Decimal::from(24850000))
        };
    ...
    fn test_parse_trade() {
        let data = FoxbitWsTrade {
            ...
            price: Some(Decimal::from(248500)), amount: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
