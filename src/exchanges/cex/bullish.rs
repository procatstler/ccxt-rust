...
impl Exchange for Bullish {
    ...
    async fn fetch_markets(&self) -> CcxtResult<Vec<Market>> {
        ...
        for market_data in response {
            ...
            let market = Market {
                ...
                settle: None,
                settle_id: None,
                active: market_data.market_enabled.unwrap_or(true),
                market_type: MarketType::Spot,
                spot: true,
                margin: market_data.margin_trading_enabled.unwrap_or(false),
                swap: false,
                future: false,
                option: false,
                index: false,
    ...
    async fn fetch_trades(
        ...
        let market_id = self.to_market_id(symbol);
        let path = format!("/v1/history/markets/{market_id}/trades");
