...
impl LatokenWs {
    ...
    async fn handle_ticker(
        ...
        let ticker = Ticker {
            ...
            high: payload.get("high").and_then(Self::parse_decimal_opt),
            low: payload.get("low").and_then(Self::parse_decimal_opt),
            bid: payload.get("bestBid").and_then(Self::parse_decimal_opt),
            bid_volume: None,
            ask: payload.get("bestAsk").and_then(Self::parse_decimal_opt),
            ask_volume: None,
            vwap: None,
            open: payload.get("open").and_then(Self::parse_decimal_opt),
            close: payload.get("lastPrice").and_then(Self::parse_decimal_opt),
            last: payload.get("lastPrice").and_then(Self::parse_decimal_opt),
            previous_close: None,
            change: payload.get("change24h").and_then(Self::parse_decimal_opt),
            percentage: None,
            average: None,
            base_volume: payload.get("volume24h").and_then(Self::parse_decimal_opt),
            quote_volume: None,
            index_price: None,
            mark_price: None,
            info: data.clone(),
