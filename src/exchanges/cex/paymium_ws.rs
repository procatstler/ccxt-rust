...
impl PaymiumWs {
    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            let currency = json.get("currency").and_then(|p| p.as_str()).unwrap_or("btceur");
            ...
            if let Some(data) = json.get("data") {
                if channel.contains("ticker") {
                    ...
                    if let Ok(book_data) = serde_json::from_value::<PaymiumWsOrderBook>(data.clone()) {
                    ...
                    if let Ok(trades_data) = serde_json::from_value::<Vec<PaymiumWsTrade>>(data.clone()) {
                    ...
                    } else if let Ok(trade_data) = serde_json::from_value::<PaymiumWsTrade>(data.clone()) {
...
impl Default for PaymiumWs { fn default() -> Self { Self::new() } }
...
struct PaymiumWsTicker {
    ...
    #[serde(default)] variation: Option<Decimal>,
}
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(PaymiumWs::format_symbol("BTC/EUR"), "btceur"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(PaymiumWs::to_unified_symbol("btceur"), "BTC/EUR"); }
    #[test] fn test_default() { let ws = PaymiumWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = PaymiumWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = PaymiumWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = PaymiumWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = PaymiumWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(40000)),
            low: Some(Decimal::from(38000)), bid: Some(Decimal::from(39500)),
            ask: Some(Decimal::from(39600)), price: Some(Decimal::from(39550)),
            volume: Some(Decimal::from(100)), vwap: None, variation: None
        };
    ...
    fn test_parse_trade() {
        let data = PaymiumWsTrade {
            uuid: Some("abc123".into()), timestamp: Some(1704067200000),
            price: Some(Decimal::from(39550)), amount: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
