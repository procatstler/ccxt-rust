...
impl TokocryptoWs {
    ...
    fn parse_trade(data: &TokocryptoWsTrade, symbol: &str) -> Trade {
        ...
        Trade {
            ...
            side: data
                .is_buyer_maker
                .map(|m| if m { "sell" } else { "buy" }.to_string()),
            taker_or_maker: None,
            price,
            amount,
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(TokocryptoWs::format_symbol("BTC/USDT"), "btcusdt");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(TokocryptoWs::to_unified_symbol("BTCUSDT"), "BTC/USDT");
    }
    #[test]
    fn test_default() {
        let ws = TokocryptoWs::default();
        assert!(ws.ws_client.is_none());
    }
    #[test]
    fn test_clone() {
        let ws = TokocryptoWs::new();
    ...
    fn test_parse_ticker() {
        let data = TokocryptoWsTicker {
            event_time: Some(1704067200000),
            high: Some(Decimal::from(50000)),
            low: Some(Decimal::from(48000)),
            open: Some(Decimal::from(49000)),
            close: Some(Decimal::from(49550)),
            bid: Some(Decimal::from(49500)),
            bid_qty: Some(Decimal::from(1)),
            ask: Some(Decimal::from(49600)),
            ask_qty: Some(Decimal::from(2)),
            volume: Some(Decimal::from(100)),
            quote_volume: Some(Decimal::from(4950000)),
            price_change: Some(Decimal::from(550)),
            price_change_percent: Some(Decimal::from(1)),
        };
    ...
    fn test_parse_order_book() {
        let data = TokocryptoWsOrderBook {
    ...
    #[test]
    fn test_parse_trade() {
        let data = TokocryptoWsTrade {
            trade_id: Some(123),
            trade_time: Some(1704067200000),
