    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            ...
            if let Some(data) = json.get("data") {
                match channel {
                    "ticker" => {
                        if let Ok(ticker_data) = serde_json::from_value::<BtcboxWsTicker>(data.clone()) {
                    ...
                    "depth" | "orderbook" => {
                        if let Ok(book_data) = serde_json::from_value::<BtcboxWsOrderBook>(data.clone()) {
                    ...
                    "trades" => {
                        if let Ok(trades_data) = serde_json::from_value::<Vec<BtcboxWsTrade>>(data.clone()) {
...
impl Default for BtcboxWs { fn default() -> Self { Self::new() } }
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(BtcboxWs::format_symbol("BTC/JPY"), "btc"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(BtcboxWs::to_unified_symbol("btc"), "BTC/JPY"); }
    #[test] fn test_default() { let ws = BtcboxWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = BtcboxWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = BtcboxWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = BtcboxWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = BtcboxWsTicker {
            high: Some(Decimal::from(5000000)), low: Some(Decimal::from(4800000)),
            buy: Some(Decimal::from(4950000)), sell: Some(Decimal::from(4960000)),
            last: Some(Decimal::from(4955000)), vol: Some(Decimal::from(100))
        };
