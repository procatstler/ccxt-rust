                ...
                match event {
                    "ticker" => {
                        if let Ok(ticker_data) =
                            serde_json::from_value::<CoinmateWsTicker>(data.clone())
                        {
                    ...
                    "order_book" | "orderbook" => {
                        if let Ok(book_data) =
                            serde_json::from_value::<CoinmateWsOrderBook>(data.clone())
                        {
                    ...
                    "trades" | "trade" => {
                        if let Ok(trades_data) =
                            serde_json::from_value::<Vec<CoinmateWsTrade>>(data.clone())
                        {
                            ...
                            serde_json::from_value::<CoinmateWsTrade>(data.clone())
                        {
...
impl WsExchange for CoinmateWs {
    ...
    async fn watch_order_book(
        ...
        let mut ws = self.clone();
        ws.subscribe_stream("order_book", &Self::format_symbol(symbol))
            .await
    }
...
struct CoinmateWsOrderBook {
    #[serde(default)]
    bids: Vec<CoinmateWsOrderBookEntry>,
    #[serde(default)]
    asks: Vec<CoinmateWsOrderBookEntry>,
}
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(CoinmateWs::format_symbol("BTC/EUR"), "BTC_EUR");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(CoinmateWs::to_unified_symbol("BTC_EUR"), "BTC/EUR");
    }
    #[test]
    fn test_default() {
        let ws = CoinmateWs::default();
        assert!(ws.ws_client.is_none());
    }
    #[test]
    fn test_clone() {
        let ws = CoinmateWs::new();
    ...
    fn test_parse_ticker() {
        let data = CoinmateWsTicker {
            ...
            bid: Some(Decimal::from(44500)),
            ask: Some(Decimal::from(44600)),
            last: Some(Decimal::from(44550)),
            open: Some(Decimal::from(44000)),
            change: None,
    ...
    fn test_parse_order_book() {
        let data = CoinmateWsOrderBook {
            bids: vec![CoinmateWsOrderBookEntry {
                price: Some(Decimal::from(44500)),
                amount: Some(Decimal::from(2)),
            }],
            asks: vec![CoinmateWsOrderBookEntry {
                price: Some(Decimal::from(44600)),
