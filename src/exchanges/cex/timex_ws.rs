...
impl TimexWs {
    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
            ...
            if let Some(payload) = json.get("payload") {
                ...
                match event {
                    ...
                    "orderbook" | "orderbook_update" | "orderbook_snapshot" => {
                        if let Ok(data) = serde_json::from_value::<TimexWsOrderBook>(payload.clone()) {
                    ...
                    "trade" | "trades" | "new_trade" => {
                        if let Ok(data) = serde_json::from_value::<Vec<TimexWsTrade>>(payload.clone()) {
                        ...
                        } else if let Ok(data) = serde_json::from_value::<TimexWsTrade>(payload.clone()) {
        ...
        if let Some(data) = json.get("data") {
            ...
            let market = json.get("market").and_then(|m| m.as_str())
...
impl Default for TimexWs { fn default() -> Self { Self::new() } }
...
struct TimexWsTicker {
    ...
    #[serde(default)] percentage: Option<Decimal>,
    #[serde(default, rename = "baseVolume")] base_volume: Option<Decimal>,
    #[serde(default, rename = "quoteVolume")] quote_volume: Option<Decimal>,
}
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(TimexWs::format_symbol("ETH/BTC"), "ETHBTC"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(TimexWs::to_unified_symbol("ETHBTC"), "ETH/BTC"); }
    #[test] fn test_to_unified_symbol_usdt() { assert_eq!(TimexWs::to_unified_symbol("BTCUSDT"), "BTC/USDT"); }
    #[test] fn test_default() { let ws = TimexWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = TimexWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = TimexWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = TimexWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = TimexWsTicker {
            timestamp: Some(1704067200000), high: Some(Decimal::from(50000)),
            low: Some(Decimal::from(48000)), bid: Some(Decimal::from(49500)),
            ask: Some(Decimal::from(49600)), last: Some(Decimal::from(49550)),
            open: Some(Decimal::from(49000)), vwap: None, change: None, percentage: None,
            base_volume: Some(Decimal::from(100)), quote_volume: Some(Decimal::from(4950000))
        };
    ...
    fn test_parse_trade() {
        let data = TimexWsTrade {
            ...
            price: Some(Decimal::from(49550)), amount: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
