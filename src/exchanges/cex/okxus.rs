//! OKX US Exchange Implementation
//!
//! Alias/wrapper for OKX exchange with different hostname (us.okx.com)
//! This is the US version of OKX, spot trading only.
...
/// OKX US 거래소 (OKX 래퍼)
///
/// OKX US는 OKX의 미국 버전으로 spot 거래만 지원합니다.
/// - API URL: <https://us.okx.com>
pub struct OkxUs {
    inner: Okx,
    urls: ExchangeUrls,
...
impl OkxUs {
    ...
    pub fn new(config: ExchangeConfig) -> CcxtResult<Self> {
        ...
        api_urls.insert("public".into(), "https://us.okx.com".into());
        api_urls.insert("private".into(), "https://us.okx.com".into());

        let urls = ExchangeUrls {
            logo: Some("https://user-images.githubusercontent.com/1294454/152485636-38b19e4a-bece-4dec-979a-5982859ffc04.jpg".into()),
            api: api_urls,
            www: Some("https://okx.com/us".into()),
            doc: vec![
                "https://www.okx.com/docs-v5/en/".into(),
            ],
            fees: Some("https://okx.com/us/fees".into()),
        };

        // OKX US is spot only - disable derivatives features
...
impl Exchange for OkxUs {
    ...
    fn sign(
        ...
        // Get signed request from inner but update URL to use OKX US API
        let mut signed = self.inner.sign(path, api, method, params, headers, body);
        signed.url = signed.url.replace("www.okx.com", "us.okx.com");
        signed
    }
...
mod tests {
    ...
    fn test_exchange_info() {
        ...
        let exchange = OkxUs::new(config).unwrap();

        assert_eq!(exchange.id(), ExchangeId::OkxUs);
        assert_eq!(exchange.name(), "OKX US");
        ...
        // Check that URLs use us.okx.com
        let urls = exchange.urls();
        assert!(urls.api.get("public").unwrap().contains("us.okx.com"));
    }
}
