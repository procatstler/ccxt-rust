            ...
            if let Some(data) = json.get("data") {
                if channel.contains("ticker") {
                    if let Ok(ticker_data) =
                        serde_json::from_value::<CryptomusWsTicker>(data.clone())
                    {
                    ...
                    if let Ok(book_data) =
                        serde_json::from_value::<CryptomusWsOrderBook>(data.clone())
                    {
                    ...
                    if let Ok(trades_data) =
                        serde_json::from_value::<Vec<CryptomusWsTrade>>(data.clone())
                    {
                        ...
                        serde_json::from_value::<CryptomusWsTrade>(data.clone())
                    {
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(CryptomusWs::format_symbol("BTC/USDT"), "BTC_USDT");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(CryptomusWs::to_unified_symbol("BTC_USDT"), "BTC/USDT");
    }
    #[test]
    fn test_default() {
        let ws = CryptomusWs::default();
        assert!(ws.ws_client.is_none());
    }
    #[test]
    fn test_clone() {
        let ws = CryptomusWs::new();
