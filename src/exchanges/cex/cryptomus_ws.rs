//! Cryptomus WebSocket Implementation
//!
//! Cryptocurrency payment gateway with exchange features
...
const WS_URL: &str = "wss://api.cryptomus.com/ws";

/// Cryptomus WebSocket client
pub struct CryptomusWs {
...
impl CryptomusWs {
    ...
    fn format_symbol(symbol: &str) -> String {
        // Cryptomus uses format like "BTC_USDT"
    ...
    fn parse_ticker(data: &CryptomusWsTicker, symbol: &str) -> Ticker {
    ...
    fn parse_order_book(data: &CryptomusWsOrderBook, symbol: &str) -> OrderBook {
        let timestamp = data.timestamp.unwrap_or_else(|| Utc::now().timestamp_millis());
    ...
    fn parse_trade(data: &CryptomusWsTrade, symbol: &str) -> Trade {
        let timestamp = data.timestamp.unwrap_or_else(|| Utc::now().timestamp_millis());
    ...
    fn process_message(msg: &str, event_tx: &mpsc::UnboundedSender<WsMessage>) -> CcxtResult<()> {
        ...
        if let Some(channel) = json.get("channel").and_then(|c| c.as_str()) {
            let pair = json.get("symbol").and_then(|p| p.as_str()).unwrap_or("");
            let symbol = Self::to_unified_symbol(pair);

            if let Some(data) = json.get("data") {
                if channel.contains("ticker") {
                    if let Ok(ticker_data) = serde_json::from_value::<CryptomusWsTicker>(data.clone()) {
                    ...
                    if let Ok(book_data) = serde_json::from_value::<CryptomusWsOrderBook>(data.clone()) {
                    ...
                    if let Ok(trades_data) = serde_json::from_value::<Vec<CryptomusWsTrade>>(data.clone()) {
                    ...
                    } else if let Ok(trade_data) = serde_json::from_value::<CryptomusWsTrade>(data.clone()) {
...
impl Default for CryptomusWs { fn default() -> Self { Self::new() } }
impl Clone for CryptomusWs {
    fn clone(&self) -> Self {
...
#[async_trait]
impl WsExchange for CryptomusWs {
...
struct CryptomusWsTicker {
    ...
    #[serde(default, rename = "changePercent")] change_percent: Option<Decimal>,
    #[serde(default)] volume: Option<Decimal>,
}

#[derive(Debug, Deserialize, Serialize)]
struct CryptomusWsOrderBook {
    #[serde(default)] timestamp: Option<i64>,
...
mod tests {
    use super::*;
    #[test] fn test_format_symbol() { assert_eq!(CryptomusWs::format_symbol("BTC/USDT"), "BTC_USDT"); }
    #[test] fn test_to_unified_symbol() { assert_eq!(CryptomusWs::to_unified_symbol("BTC_USDT"), "BTC/USDT"); }
    #[test] fn test_default() { let ws = CryptomusWs::default(); assert!(ws.ws_client.is_none()); }
    #[test] fn test_clone() { let ws = CryptomusWs::new(); assert!(ws.clone().ws_client.is_none()); }
    #[test] fn test_new() { let ws = CryptomusWs::new(); assert!(ws.ws_client.is_none()); }
    #[tokio::test] async fn test_ws_is_connected() { let ws = CryptomusWs::new(); assert!(!ws.ws_is_connected().await); }
    #[test]
    fn test_parse_ticker() {
        let data = CryptomusWsTicker {
            ...
            change: None, change_percent: None, volume: Some(Decimal::from(100))
        };
        let ticker = CryptomusWs::parse_ticker(&data, "BTC/USDT");
        assert_eq!(ticker.symbol, "BTC/USDT");
    }
    #[test]
    fn test_parse_order_book() {
        let data = CryptomusWsOrderBook {
            timestamp: Some(1704067200000),
            bids: vec![vec!["44500".into(), "1.5".into()]],
            asks: vec![vec!["44600".into(), "1.0".into()]]
        };
        let ob = CryptomusWs::parse_order_book(&data, "BTC/USDT");
        assert_eq!(ob.bids.len(), 1);
    }
    #[test]
    fn test_parse_trade() {
        let data = CryptomusWsTrade {
            id: Some("123".into()), timestamp: Some(1704067200000),
            price: Some(Decimal::from(44550)), amount: Some(Decimal::from(1)),
            side: Some("buy".into())
        };
        let trade = CryptomusWs::parse_trade(&data, "BTC/USDT");
        assert_eq!(trade.id, "123");
    }
    #[test]
    fn test_process_message() {
        let (tx, _rx) = mpsc::unbounded_channel();
        let msg = r#"{"channel":"ticker","symbol":"BTC_USDT","data":{"last":"44550"}}"#;
        assert!(CryptomusWs::process_message(msg, &tx).is_ok());
    }
}
