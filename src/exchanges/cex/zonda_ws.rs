                ...
                if action == "push" {
                    if topic.contains("ticker") {
                        if let Ok(ticker_data) =
                            serde_json::from_value::<ZondaWsTicker>(message.clone())
                        {
                        ...
                        if let Ok(book_data) =
                            serde_json::from_value::<ZondaWsOrderBook>(message.clone())
                        {
                        ...
                        if let Ok(trades_data) =
                            serde_json::from_value::<Vec<ZondaWsTrade>>(message.clone())
                        {
                            ...
                            serde_json::from_value::<ZondaWsTrade>(message.clone())
                        {
...
impl WsExchange for ZondaWs {
    ...
    async fn watch_order_book(
        ...
        let mut ws = self.clone();
        ws.subscribe_stream("orderbook-limited/10", &Self::format_symbol(symbol))
            .await
    }
    async fn watch_trades(&self, symbol: &str) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        let mut ws = self.clone();
        ws.subscribe_stream("transactions", &Self::format_symbol(symbol))
            .await
    }
...
struct ZondaWsTicker {
    ...
    #[serde(default)]
    rate: Option<Decimal>,
    #[serde(default, rename = "previousRate")]
    previous_rate: Option<Decimal>,
...
struct ZondaWsOrderBook {
    ...
    #[serde(default)]
    buy: Vec<ZondaWsOrderBookEntry>,
    #[serde(default)]
    sell: Vec<ZondaWsOrderBookEntry>,
}
...
mod tests {
    ...
    fn test_format_symbol() {
        assert_eq!(ZondaWs::format_symbol("BTC/PLN"), "BTC-PLN");
    }
    #[test]
    fn test_to_unified_symbol() {
        assert_eq!(ZondaWs::to_unified_symbol("BTC-PLN"), "BTC/PLN");
    }
    #[test]
    fn test_default() {
        let ws = ZondaWs::default();
    ...
    fn test_parse_ticker() {
        let data = ZondaWsTicker {
            time: Some(1704067200000),
            highest_bid: Some(Decimal::from(180000)),
            lowest_ask: Some(Decimal::from(181000)),
            rate: Some(Decimal::from(180500)),
            previous_rate: Some(Decimal::from(179000)),
            high_price: Some(Decimal::from(185000)),
            low_price: Some(Decimal::from(175000)),
    ...
    fn test_process_message() {
        ...
        let msg =
            r#"{"action":"push","topic":"trading/ticker/BTC-PLN","message":{"rate":"180500"}}"#;
