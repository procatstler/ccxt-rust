...
use crate::types::{
    Balance, Balances, Order, OrderStatus, OrderSide, OrderType, TimeInForce,
    WsBalanceEvent, WsOrderBookEvent, WsOrderEvent, WsOhlcvEvent, OrderBook, OrderBookEntry,
    Ticker, WsTickerEvent, Timeframe, Trade, WsTradeEvent, WsExchange, WsMessage, TakerOrMaker,
};
...
impl HashkeyWs {
    ...
    async fn connect_private(&self) -> CcxtResult<WsClient> {
        let (ws_stream, _) = connect_async(WS_PRIVATE_URL)
            .await
            .map_err(|e| CcxtError::NetworkError { url: WS_PRIVATE_URL.to_string(), message: e.to_string() })?;
        Ok(ws_stream)
    }
    ...
    async fn subscribe_private_stream(&self, ws: &mut WsClient, channel: &str) -> CcxtResult<()> {
        ...
        let sub_str = serde_json::to_string(&sub_msg)
            .map_err(|e| CcxtError::ParseError { data_type: "json".to_string(), message: e.to_string() })?;
    ...
    fn process_private_message(&self, msg: &str) -> Option<WsMessage> {
        ...
        match event_type {
            "executionReport" => {
                // Order update
                if let Ok(order_data) = serde_json::from_value::<HashkeyOrderUpdateData>(parsed.clone()) {
    ...
    fn parse_order(&self, data: &HashkeyOrderUpdateData) -> Option<WsMessage> {
        ...
        let filled = data.cumulative_filled_qty.as_ref()
        ...
        let order = Order {
            ...
            side,
            price,
            average: data.last_filled_price.as_ref().and_then(|p| Decimal::from_str(p).ok()),
    ...
    fn parse_balance(&self, balances_arr: &[serde_json::Value], parsed: &serde_json::Value) -> Option<WsMessage> {
        let mut currencies: HashMap<String, Balance> = HashMap::new();

        for bal in balances_arr {
            if let Ok(balance_data) = serde_json::from_value::<HashkeyBalanceUpdateData>(bal.clone()) {
        ...
        let timestamp = parsed.get("E").and_then(|v| v.as_i64())
...
impl WsExchange for HashkeyWs {
    ...
    async fn watch_orders(&self, _symbol: Option<&str>) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        ...
        self.subscribe_private_stream(&mut ws, "executionReport").await?;
                    ...
                    Ok(Message::Text(text)) => {
                        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&text) {
                            if parsed.get("e").and_then(|v| v.as_str()) == Some("executionReport") {
                                if let Ok(data) = serde_json::from_value::<HashkeyOrderUpdateData>(parsed.clone()) {
                                    if let Some(symbol_raw) = &data.symbol {
                                        ...
                                        let filled = data.cumulative_filled_qty.as_ref().and_then(|f| Decimal::from_str(f).ok());
    ...
    async fn watch_balance(&self) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        ...
        self.subscribe_private_stream(&mut ws, "outboundAccountPosition").await?;
            ...
            while let Some(msg) = ws.next().await {
                match msg {
                    Ok(Message::Text(text)) => {
                        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&text) {
                            let event_type = parsed.get("e").and_then(|v| v.as_str());
                            if event_type == Some("outboundAccountPosition") || event_type == Some("balanceUpdate") {
                                if let Some(balances_arr) = parsed.get("B").and_then(|v| v.as_array()) {
                                    let mut currencies: HashMap<String, Balance> = HashMap::new();
...
mod tests {
    ...
    fn test_parse_order_status_mapping() {
        ...
        for (status_str, expected_status) in test_cases {
            ...
            let result = ws.parse_order(&order_data);
            if let Some(WsMessage::Order(event)) = result {
                assert_eq!(event.order.status, expected_status, "Failed for status: {status_str}");
            }
        }
    }
