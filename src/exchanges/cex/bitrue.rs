...
impl Bitrue {
    const BASE_URL: &'static str = "https://www.bitrue.com/api";
    const RATE_LIMIT_MS: u64 = 10; // 10ms rate limit
    ...
    pub fn new(config: ExchangeConfig) -> CcxtResult<Self> {
        ...
        api_urls.insert("spot".into(), "https://www.bitrue.com/api".into());
        api_urls.insert("fapi".into(), "https://fapi.bitrue.com/fapi".into());
        ...
        let urls = ExchangeUrls {
            ...
            doc: vec![
                "https://github.com/Bitrue-exchange/bitrue-official-api-docs".into(),
                "https://www.bitrue.com/api-docs".into(),
            ],
            fees: Some("https://bitrue.zendesk.com/hc/en-001/articles/4405479952537".into()),
        };
    ...
    /// 주문 응답 파싱
    fn parse_order(&self, data: &BitrueOrder, symbol: &str) -> Order {
    ...
    /// 잔고 응답 파싱
    fn parse_balance(&self, balances: &[BitrueBalance]) -> Balances {
    ...
    /// 입금 내역 파싱
    fn parse_deposit(&self, data: &BitrueDeposit) -> Transaction {
    ...
    /// 출금 내역 파싱
    fn parse_withdrawal(&self, data: &BitrueWithdrawal) -> Transaction {
        let status = match data.status {
            0 => crate::types::TransactionStatus::Pending,
...
impl Exchange for Bitrue {
    ...
    async fn fetch_markets(&self) -> CcxtResult<Vec<Market>> {
        ...
        for symbol_info in response.symbols {
            ...
            let market = Market {
                ...
                inverse: None,
                sub_type: None,
                taker: Some(Decimal::new(98, 5)), // 0.00098
                maker: Some(Decimal::new(98, 5)), // 0.00098
                contract_size: None,
                expiry: None,
    ...
    async fn fetch_open_orders(
        ...
        let response: Vec<BitrueOrder> = self
            .private_request("GET", "/v1/openOrders", params)
            .await?;
    ...
    async fn fetch_my_trades(
        ...
        let symbol = symbol.ok_or_else(|| CcxtError::ArgumentsRequired {
            message: "Bitrue fetchMyTrades requires a symbol".into(),
        })?;
    ...
    async fn fetch_closed_orders(
        ...
        let symbol = symbol.ok_or_else(|| CcxtError::ArgumentsRequired {
            message: "Bitrue fetchClosedOrders requires a symbol".into(),
        })?;
    ...
    async fn fetch_deposits(
        ...
        let response: Vec<BitrueDeposit> = self
    ...
    async fn fetch_withdrawals(
        ...
        let response: Vec<BitrueWithdrawal> = self
...
// === Bitrue API Response Types ===

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct BitrueExchangeInfo {
    symbols: Vec<BitrueSymbol>,
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct BitrueAccountInfo {
    balances: Vec<BitrueBalance>,
}

#[derive(Debug, Deserialize)]
struct BitrueBalance {
    asset: String,
    free: String,
    locked: String,
}
...
mod tests {
    ...
    fn test_symbol_conversion() {
        ...
        let bitrue = Bitrue::new(config).unwrap();

        assert_eq!(bitrue.to_market_id("BTC/USDT"), "BTCUSDT");
        assert_eq!(bitrue.to_market_id("ETH/BTC"), "ETHBTC");
    }
    ...
    fn test_exchange_info() {
        ...
        assert_eq!(bitrue.id(), ExchangeId::Bitrue);
        assert_eq!(bitrue.name(), "Bitrue");
        assert!(bitrue.has().spot);
        assert!(bitrue.has().swap);
    }
}
