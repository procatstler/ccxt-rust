...
impl Poloniex {
    ...
    fn sign_request(&self, method: &str, path: &str, timestamp: &str, body: &str) -> CcxtResult<String> {
        ...
        if method == "POST" || method == "PUT" || method == "DELETE" {
            ...
            // GET request
            if !body.is_empty() {
                auth.push_str(&format!("{}&signTimestamp={}", body, timestamp));
            } else {
                auth.push_str(&format!("signTimestamp={}", timestamp));
            }
        }
...
impl Exchange for Poloniex {
    ...
    async fn fetch_ticker(&self, symbol: &str) -> CcxtResult<Ticker> {
        ...
        let path = format!("/markets/{}/ticker24h", poloniex_symbol);
    ...
    async fn fetch_order_book(&self, symbol: &str, limit: Option<u32>) -> CcxtResult<OrderBook> {
        let poloniex_symbol = self.to_market_id(symbol);
        let path = format!("/markets/{}/orderBook", poloniex_symbol);
    ...
    async fn fetch_trades(&self, symbol: &str, since: Option<i64>, limit: Option<u32>) -> CcxtResult<Vec<Trade>> {
        let poloniex_symbol = self.to_market_id(symbol);
        let path = format!("/markets/{}/trades", poloniex_symbol);
    ...
    async fn fetch_ohlcv(
        ...
        let path = format!("/markets/{}/candles", poloniex_symbol);
