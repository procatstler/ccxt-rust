...
impl P2bWs {
    ...
    async fn handle_ticker(
        ...
        let market_id = params.get(0).and_then(|v| v.as_str()).unwrap_or("");
    ...
    async fn handle_orderbook(
        ...
        let is_snapshot = params.get(0).and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        if let Some(asks_arr) = ob_data.get("asks").and_then(|v| v.as_array()) {
            for entry in asks_arr {
                if let Some(arr) = entry.as_array() {
                    let price: Decimal = arr.get(0).and_then(|v| v.as_str())
    ...
    async fn handle_ohlcv(
        ...
        if let Some(kline) = params.get(0).and_then(|v| v.as_array()) {
            // [timestamp, open, close, high, low, vol_stock, vol_money, market]
            let timestamp = kline.get(0).and_then(|v| v.as_i64())
