...
impl DeltaWs {
    ...
    async fn authenticate(&self) -> CcxtResult<()> {
        ...
        let signature_payload = format!("GET{}/live", timestamp);
    ...
    async fn handle_ticker(
        ...
        let ticker_data = data.get("data").or(Some(data)).unwrap();
        ...
        let ticker = Ticker {
            ...
            high: ticker_data.get("high").and_then(|v| Self::parse_decimal_opt(v)),
            low: ticker_data.get("low").and_then(|v| Self::parse_decimal_opt(v)),
            bid: ticker_data.get("best_bid").and_then(|v| Self::parse_decimal_opt(v)),
            bid_volume: None,
            ask: ticker_data.get("best_ask").and_then(|v| Self::parse_decimal_opt(v)),
            ask_volume: None,
            vwap: None,
            open: ticker_data.get("open").and_then(|v| Self::parse_decimal_opt(v)),
            close: ticker_data.get("close").and_then(|v| Self::parse_decimal_opt(v)),
            last: ticker_data.get("close").and_then(|v| Self::parse_decimal_opt(v)),
            previous_close: None,
            change: None,
            percentage: ticker_data.get("price_change_percent_24h")
                .and_then(|v| Self::parse_decimal_opt(v)),
            average: None,
            base_volume: ticker_data.get("volume").and_then(|v| Self::parse_decimal_opt(v)),
            quote_volume: ticker_data.get("turnover").and_then(|v| Self::parse_decimal_opt(v)),
            index_price: ticker_data.get("spot_price").and_then(|v| Self::parse_decimal_opt(v)),
            mark_price: ticker_data.get("mark_price").and_then(|v| Self::parse_decimal_opt(v)),
            info: data.clone(),
        };
    ...
    async fn handle_orderbook(
        ...
        let ob_data = data.get("data").or(Some(data)).unwrap();
    ...
    async fn handle_ohlcv(
        ...
        let candle_data = data.get("data").or(Some(data)).unwrap();
