...
impl AscendexWs {
    ...
    async fn get_account_group(&self) -> CcxtResult<String> {
        ...
        let url = format!("{}/api/pro/v1/info", REST_BASE_URL);
    ...
    async fn subscribe_private_stream(&mut self, channel: &str) -> CcxtResult<mpsc::UnboundedReceiver<WsMessage>> {
        ...
        let url = format!("wss://ascendex.com:443/{}/api/pro/v2/stream", account_group);
        ...
        // Authenticate
        let timestamp = Utc::now().timestamp_millis().to_string();
        let path = format!("/{}/api/pro/v2/stream", account_group);
        let version = "v2";
    ...
    fn parse_ticker(data: &AscendexBboMessage) -> WsTickerEvent {
        ...
        let ticker = Ticker {
            ...
            high: None,
            low: None,
            bid: data.data.bid.get(0).and_then(|v| v.parse().ok()),
            bid_volume: data.data.bid.get(1).and_then(|v| v.parse().ok()),
            ask: data.data.ask.get(0).and_then(|v| v.parse().ok()),
