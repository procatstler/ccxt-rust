...
impl KrakenFutures {
    ...
    pub fn new(config: ExchangeConfig) -> CcxtResult<Self> {
        ...
        api_urls.insert("public".into(), format!("{}/v3", BASE_URL));
        api_urls.insert("private".into(), format!("{}/v3", BASE_URL));
...
impl Exchange for KrakenFutures {
    ...
    async fn fetch_balance(&self) -> CcxtResult<Balances> {
        ...
        for (_account_type, account) in &data.data.accounts {
    ...
    async fn create_order(
        ...
        let order_type_str = match order_type {
            ...
            _ => return Err(CcxtError::NotSupported { feature: format!("Order type {:?} not supported", order_type) }),
        };
