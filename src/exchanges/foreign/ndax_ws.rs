                        ...
                        if let Some(ticker) = ticker_array.as_array() {
                            if ticker.len() >= 10 {
                                ...
                                if let Some(symbol) = symbol_opt {
                                    ...
                                    let high: Option<Decimal> = ticker[2].as_f64().and_then(|f| Decimal::from_f64(f));
                                    let low: Option<Decimal> = ticker[3].as_f64().and_then(|f| Decimal::from_f64(f));
                                    let open: Option<Decimal> = ticker[4].as_f64().and_then(|f| Decimal::from_f64(f));
                                    let close: Option<Decimal> = ticker[5].as_f64().and_then(|f| Decimal::from_f64(f));
                                    let volume: Option<Decimal> = ticker[6].as_f64().and_then(|f| Decimal::from_f64(f));
