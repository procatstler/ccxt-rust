...
impl Bit2c {
    ...
    async fn public_request(&self, path: &str) -> CcxtResult<serde_json::Value> {
        ...
        let url = format!("{}/{}.json", BASE_URL, path);
...
impl Exchange for Bit2c {
    ...
    async fn fetch_ticker(&self, symbol: &str) -> CcxtResult<Ticker> {
        ...
        let path = format!("Exchanges/{}/Ticker", market_id);
    ...
    async fn fetch_order_book(&self, symbol: &str, limit: Option<u32>) -> CcxtResult<OrderBook> {
        let market_id = self.get_market_id(symbol)?;
        let path = format!("Exchanges/{}/orderbook", market_id);
    ...
    async fn fetch_trades(
        ...
        let market_id = self.get_market_id(symbol)?;
        let mut path = format!("Exchanges/{}/trades", market_id);
