...
impl DydxMarketInfo {
    ...
    pub fn size_to_quantums(&self, size: Decimal) -> u64 {
        ...
        let rounded = (quantums.round() * Decimal::from(self.step_base_quantums))
            .to_string()
            .parse::<u64>()
            .unwrap_or(0);
        rounded
    ...
    pub fn price_to_subticks(&self, price: Decimal) -> u64 {
        ...
        let rounded = (ticks * Decimal::from(self.subticks_per_tick))
            .to_string()
            .parse::<u64>()
            .unwrap_or(0);
        rounded
...
impl DydxNodeClient {
    ...
    pub async fn broadcast_tx_sync(&self, tx_bytes: &[u8]) -> CcxtResult<BroadcastTxSyncResult> {
        ...
        if code != 0 {
            ...
            return Err(crate::errors::CcxtError::ExchangeError {
                message: format!("Transaction failed: code={}, log={}", code, log),
            });
        }
