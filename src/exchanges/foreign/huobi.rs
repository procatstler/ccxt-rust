...
//!
//! Alias for HTX exchange (same API, previous branding name)
...
/// Huobi 거래소 (HTX 래퍼)
///
/// Huobi는 HTX의 이전 브랜드명으로, 동일한 API를 사용합니다.
/// HTX는 2023년에 Huobi에서 리브랜딩되었습니다.
pub struct Huobi(Htx);

impl Huobi {
    /// 새 Huobi 인스턴스 생성
    pub fn new(config: ExchangeConfig) -> CcxtResult<Self> {
        Ok(Self(Htx::new(config)?))
    }
}
...
mod tests {
    ...
    fn test_exchange_info() {
        ...
        let exchange = Huobi::new(config).unwrap();

        assert_eq!(exchange.id(), ExchangeId::Huobi);
        assert_eq!(exchange.name(), "Huobi");
